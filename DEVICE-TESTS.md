...
Written against app **v0.30**. `node smoke-test.mjs` checks the pure logic
...
| D2 | The app starts at all | Watch the console through the first render | The startup banner names `v0.30` and nothing throws. This is the class of failure that shipped in v0.23 |
...
None yet. `v0.30` was verified in one desktop browser only, so the matrix is
